...
In a very easy way (though a JSON config file) you can configure:
 ...
 - <b>Authentication mode</b>: you can choose to enable/disable the authentication nad you can setup it though JSON Web Token or basic-auth.
 - <b>Socket.io</b>: you can enable/disable this capability

Basically, this project has been developed around the following modules:
...
<br/>You can find this information under the object server as follow:
...
```
where
- <b>port</b>: is the server listening port
...
<br/>You can find this information under the object database as follow:
...
- <b>user</b>: user name to login into you database
- <b>password</b>: password to login into you database
...
- <b>isEnabled</b>: set this param to false if you don't want to use and to ocnnect to the database.
...
- <b>jwt.cryptography</b>: is the kind of encryption that you want to use for the token: 0 for a symmetric encryption with a key, 1 to use an encryption based on a public/private key pair.
- <b>jwt.symmetricKey</b>:key used for the token cryptography.
...
<br/>You can find this information under the object socketio as follow:
...
Why didi I use Socket.io in for RESTFul web services? 
<br/>Well, I think that it should be useful to implement a real-time communication with other actors in a more complex architecture.
Imagine a similar scenario:

![ScreenShot](https://raw.github.com/alchimya/DoYouREST/master/screenshots/rest_socket.png)

In this scenario a mobile application executes the login though a web api, the server will dispatch a signal, that a new client is connected, to the web app monitor connected to the same server through a socket.io instance.
<br/>
In this way you can monitor all the activities and actions of each client connected to the server.
<br/>
Imagine that you have a mobile app with a “check-in” function like Facebook. 
<br/>
When the use perform the operation, by calling a web api, sends a request to the server that performs 
its operations and dispatches to the web monitor clients a message with the latitude and longitude of the mobile 
device that could be shown in a map.
...
With this package, is quite easy to add a new REST resource module to serve your custom API.
...
Within the folder controllers, you will find some test js files, that implement different REST api.
...
When in a clien folder you will find a package.json file (e.g. Angular and jQuery), it means that you have to install it though npm.
